...
# --- Reusable API Client ---
client = AsyncOpenAI(api_key=ELECTRON_API_KEY, base_url=ELECTRON_BASE_URL, timeout=60.0)
...
async def electron_gemini(bot: BOT, message: Message):
    ...
    # Only if running in raw/enhanced mode (-r or -raw)
    if "-r" in message.flags or "-raw" in message.flags:
        try:
            ...
            if (
                enhanced_prompt
                and len(enhanced_prompt) > 5
                and "Error" not in enhanced_prompt
            ):
        ...
        try:
            await wait_message.edit_media(
                media=InputMediaPhoto(
                    ...
                    caption=f"**Response:**\n{text}"[
                        :1024
                    ],  # Caption has a 1024 char limit
                    parse_mode=ParseMode.MARKDOWN,
                )
            )
        except Exception as e:
            await wait_message.edit(
                f"**Generated Image** (Upload failed: `{e}`)\n\n{text}"
            )
    elif text:
...
async def gpt5_text(bot: BOT, message: Message):
    ...
    try:
        text, _, error = await asyncio.wait_for(
            generate(
                final_prompt, image_file, model=MODEL_TEXT, system_prompt=SYSTEM_PROMPT
            ),
            timeout=45.0,
        )
